...
def get_org_customization():
    print "get_org_customization: {0}".format(config.okta["org_host"])
    page = requests.get(config.okta["org_host"])
    s = page.content
    startT = "logo: '"
    endT = '\''
    logo = find_between(s,startT,endT)

    startT = "background-image: url('"
    endT = '\''
    backgroundImage = find_between(s,startT,endT)
    print "logo = ",logo
    print "backgroundImage = ",backgroundImage
    app.orgUrls = {"logo":logo, "background":backgroundImage}
    return
